...
class ProductControllerTest {
    ...
    void listProducts_authenticated_returns200() throws Exception {
        given(productService.getProducts(any())).willReturn(
            new PageImpl<>(java.util.List.of(), PageRequest.of(0, 5), 0)
        );
    ...
    @Test
    @WithMockUser(roles = "USER")
    @DisplayName("인증된 사용자 - 상품 검색 조회 성공 (200)")
    void searchProducts_authenticated_returns200() throws Exception {
        given(productService.searchProducts(any(), any())).willReturn(
            new PageImpl<>(java.util.List.of(), PageRequest.of(0, 5), 0)
        );
    ...
    void editForm_admin_returns200() throws Exception {
        given(productService.findById(1L)).willReturn(
            new Product("기존 상품", 15000, "기존 설명", 10)
        );
    ...
    void editProduct_admin_redirectsToList() throws Exception {
        given(productService.updateProduct(anyLong(), any())).willReturn(
            new Product("수정 상품", 20000, "수정 설명", 7)
        );
    ...
    void editProduct_validationError_returnsEditForm() throws Exception {
        mockMvc.perform(post("/products/1/edit")
            ...
            .andExpect(view().name("products/edit"))
            .andExpect(model().attribute("productId", 1L))
            .andExpect(model().attributeHasFieldErrors("productDto", "name", "price", "stock"));

        then(productService).should(never()).updateProduct(anyLong(), any());
    }
